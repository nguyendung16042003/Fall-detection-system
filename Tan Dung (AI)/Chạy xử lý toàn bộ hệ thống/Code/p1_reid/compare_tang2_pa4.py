"""Tier-2 TPR test cho OSNet x0.25 Phuong an 4 (Teacher Assistant, kd_weight=0.3, Teacher=TA x0.5)
epoch), CUNG data/quy trinh voi compare_tang2_baseline.py: ca 2 camera
...
BASELINE_CKPT = Path(__file__).resolve().parent / "osnet_x0_25_kd_pa4_via_ta_msmt17_best.pt"
ENROLLMENT = {
...
def main():
    print("Nap OSNet x0.25 Phuong an 4 (via Teacher Assistant)...")
    ...
    print("\nDanh gia OSNet x0.25 Phuong an 4 (ca 2 camera x 6 bien the)...")
    rows = evaluate_model("osnet_x0_25_kd_pa4", embed, detector, gallery)

    df = pd.DataFrame(rows)
    out_csv = RESULTS_DIR / "p1_tang2_pa4.csv"
