"""
P1 -- video demo GOP: ReID (OSNet x0.5, dung LAI nguyen ham build_gallery/
match_gallery/crop_person cua make_p1_scene5_both_cams_video.py, KHONG sua) +
RULE THAT (Coding/Pipeline/fall_rule.py) chay NGAM o hau truong.

Theo yeu cau: video CHI hien thi nhan danh tinh "Person N" (ket qua ReID) de
kiem tra ReID co on dinh khong -- KHONG hien thi nhan tu the (da xac nhan tu
the nhay loan khi nguoi dang di lai, la han che that cua classifier, khong
anh huong toi rule vi rule chi quan tam chuoi 'lie' lien tuc). Tu the van
duoc phan loai VA dua vao rule NGAM (khong ve len video), banner do "!!! FALL
DETECTED !!!" van hien neu rule trigger that.
"""
import sys
...
from make_p1_scene5_both_cams_video import (  # noqa: E402
    ENROLLMENT, OSNET_CKPT, DET_CONF, crop_person, build_gallery, match_gallery,
)
...
def analyze_video(video_path, embed_fn, backbone, detector, gallery, next_wrong_label_start=2):
    """Nhu analyze_video() goc (ReID) + CHAY NGAM phan loai tu the -> pose_records
    cho rule that. Video KHONG ve nhan tu the, chi ve nhan ReID."""
...
def main():
    ...
    print("Nap OSNet x0.5 + SharedBackbone (posture, chay ngam) + detector...")
    ...
    print(f"\nTONG ReID: {n_correct}/{n_total} dung")
    ...
    out_path = OUT_DIR / "P1_Scene5_ReID_rule_ghep.mp4"
