"""
Adapter: noi chuoi phan loai tu the DA FUSE (Boundary Feature Fusion) vao
dung RULE THAT cua he thong -- fall_rule.py, ham detect_fall_events(). KHONG
viet lai rule, chi tai dung nguyen ban (fall_rule.py thuoc Part 1
-- 1_Single_Camera_Pipeline/ -- xem README.md cua Part 2, muc "Phu thuoc
Part 1").
...
video), trong khi Boundary Feature Fusion gop 2 camera truoc khi phan loai --
moi "frame" la 1 thoi diem DA FUSE, dai dien cho 1 nguoi DUY NHAT (khong con
tach theo tung cam). Kien truc goc (Context toan bo he thong.docx) xac nhan
dung thiet ke nay: "Temporal Rule Detector chay SAU buoc gop, tren nhan da
hop nhat theo dinh danh nguoi -- neu chay truoc khi gop, ca nga dung vung
giao 2 cam se KHONG bao gio tao duoc chuoi stand/sit->lie".
"""
import sys
from pathlib import Path

# fall_rule.py thuoc Part 1 (1_Single_Camera_Pipeline/pipeline_code/) --
# gia dinh Part 1 va Part 2 la 2 thu muc con cung 1 thu muc cha (nhu luc
# ban giao). Neu tach roi, sua duong dan nay cho khop.
PART1_PIPELINE_CODE = Path(__file__).resolve().parents[3] / "1_Single_Camera_Pipeline" / "pipeline_code"
sys.path.insert(0, str(PART1_PIPELINE_CODE))
