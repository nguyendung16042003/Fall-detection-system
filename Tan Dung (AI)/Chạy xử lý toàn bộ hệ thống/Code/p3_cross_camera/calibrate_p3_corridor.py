...
P3 Cam2 (bep) KHONG can calib lai -- dung nguyen homography_matrices.json /
homography_head_matrices.json cua P2 CAM 1 (da xac nhan cung 1 camera vat
ly, khong doi vi tri, xem hoi thoai).
"""
import json
import sys
...
CALIB_VIDEO = (AI_ROOT / "Chạy xử lý toàn bộ hệ thống" / "File Run Problem 3" / "P3"
               / "P3 Data Calib" / "Data Calib ( hướng ra hành lang).avi")
...
def main():
    ...
    with open(RESULTS_P2 / "homography_matrices.json", encoding="utf-8") as f:
        p2_floor = json.load(f)
    with open(RESULTS_P2 / "homography_head_matrices.json", encoding="utf-8") as f:
        p2_head = json.load(f)
    ...
    with open(RESULTS_P3 / "homography_matrices_p3.json", "w", encoding="utf-8") as f:
        json.dump(out_floor, f, indent=2)
    with open(RESULTS_P3 / "homography_head_matrices_p3.json", "w", encoding="utf-8") as f:
        json.dump(out_head, f, indent=2)
    print(f"\nDa luu: {RESULTS_P3 / 'homography_matrices_p3.json'}")
    print(f"Da luu: {RESULTS_P3 / 'homography_head_matrices_p3.json'}")
