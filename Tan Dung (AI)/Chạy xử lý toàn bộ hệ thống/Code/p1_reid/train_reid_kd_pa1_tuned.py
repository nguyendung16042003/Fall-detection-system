"""
P1 -- Phuong an 1 (Dieu chinh Hyperparameter KD), dung DUNG cau hinh da chon
voi nguoi dung: kd_weight=0.2 (giam tu 0.5 goc) + warm-up 25 epoch dau (kd
weight=0 trong warm-up, coi nhu dang train Baseline), GIU NGUYEN MSE loss
cho L_KD (khong doi sang cosine). Xem
"File Run Problem 1/PhuongAn1_Dieu_chinh_Hyperparameter_KD.docx" muc 1+2
(to hop manh nhat guide tu de xuat).

Moi thu khac GIU NGUYEN 100% so voi train_reid_kd_osnet025.py (Teacher
x1.0 dong bang, dataset/split, transform, kien truc, optimizer/scheduler,
early-stop).
"""
import sys
import time
...
STUDENT_OUT = HERE / "osnet_x0_25_kd_pa1_msmt17.pth"
RESUME_CKPT = RESULTS_DIR / "osnet_x0_25_kd_pa1_checkpoint_latest.pth"
BEST_STUDENT_OUT = HERE / "osnet_x0_25_kd_pa1_msmt17_best.pt"
...
WEIGHT_DECAY = 5e-4
MARGIN = 0.3
W_ID, W_TRIPLET = 1.0, 1.0
KD_WEIGHT = 0.2          # giam tu 0.5 (Phuong an 1 muc 1)
WARMUP_EPOCHS = 25       # Phuong an 1 muc 2
MILESTONES = [40, 70]
GAMMA = 0.1
CKPT_EVERY = 5
...
def main():
    ...
    print(f"  KD_WEIGHT={KD_WEIGHT}, WARMUP_EPOCHS={WARMUP_EPOCHS} (kd_weight=0 trong warmup)")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        current_kd_weight = 0.0 if epoch < WARMUP_EPOCHS else KD_WEIGHT
        ...
        for imgs, labels in loader:
            ...
            total_loss = W_ID * loss_ce + W_TRIPLET * loss_tri + current_kd_weight * loss_kd
        ...
        print(f"Epoch {epoch:3d}/{EPOCHS}  kd_w={current_kd_weight:.2f}  loss_ce={total_ce/n_batches:.4f}  "
