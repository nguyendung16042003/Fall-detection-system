"""Tier-2 TPR test cho MSINet (doi thu 2), CUNG data/quy trinh voi
compare_tang2_full_kd025.py (osnet_x0_25_kd vs shared_backbone) de so sanh
cong bang: ca 2 camera Scene 5 x 6 bien the (goc + 5 augment) = 24 case."""
import sys
...
def main():
    print("Nap MSINet (checkpoint pretrained tai ve, khong tu train)...")
    detector = YOLO(str(AI_ROOT / "yolov8n.pt"))
    msinet_embed = make_msinet_embed_fn(device="cpu")

    print("Xay Gallery...")
    gallery = build_gallery(msinet_embed, detector)

    print("\nDanh gia MSINet (ca 2 camera x 6 bien the)...")
    rows = evaluate_model("msinet", msinet_embed, detector, gallery)

    df = pd.DataFrame(rows)
    out_csv = RESULTS_DIR / "p1_tang2_msinet.csv"
