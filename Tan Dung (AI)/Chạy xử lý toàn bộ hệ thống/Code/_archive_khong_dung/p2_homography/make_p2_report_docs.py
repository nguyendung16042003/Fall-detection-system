"""Tao 3 file Word (Bai cua nhom / Doi thu / So sanh) cho report P2, cung cau
truc voi make_p1_report_docs.py / make_p3_report_docs.py."""
import sys
...
BASE = AI_ROOT / "Chạy xử lý toàn bộ hệ thống" / "Số liệu thống kê" / "P2"
...
def build_doc_own():
    doc = Document()
    doc.add_heading("P2 — Cross-Camera Identity Association: Bài của nhóm", level=0)
    ...
    doc.add_paragraph(
        "Test trên P2 Data 3 (4 cảnh, nhiều người) — bộ test khó nhất của P2, có cả tình huống "
        "đứng sát nhau, che khuất, và ngã. ĐẦY ĐỦ: bản gốc + 4 biến thể augment an toàn với "
        "homography (sáng/tối/nhiễu/nén — không lật/xoay/crop vì P2 phụ thuộc calib pixel gốc, "
        "giống quy ước đã dùng cho P1/P3) — tổng 4 cảnh × 5 biến thể = 840 frame (2 người/frame):"
    )
    ...
    doc.add_paragraph(
        "Lưu ý QUAN TRỌNG về ground truth: P2 Data 3 không có nhãn danh tính gán tay sẵn. Ground "
    ...
    doc.add_paragraph(
        ...
        "appearance + học — P2 giải quyết được phần lớn chỉ bằng cách chọn đúng thuật toán gán "
...
def build_doc_competitor():
    doc = Document()
    doc.add_heading("P2 — Cross-Camera Identity Association: Đối thủ (GNN-CCA, Luna et al. 2022)", level=0)
    ...
    doc.add_paragraph(
        ...
        "P2) → Message Passing Network (MPN, học lan truyền đặc trưng qua đồ thị) → phân loại "
        "cạnh active/inactive → hậu xử lý (pruning + splitting) → Connected Components ra danh "
        "tính cuối."
    )
    doc.add_paragraph(
        "GIỚI HẠN THẬT — không train/chạy được bản GNN học đầy đủ trên data P2: (1) cần TRAIN có "
        ...
        "— tái hiện lại và chạy trên data P2 thật (xem folder 3)."
    )
    ...
    doc.add_paragraph(
        ...
        "4 camera — khó hơn nhiều so với P2: 1-2 người, 2 camera), trung bình 3 tập train-test "
    ...
    add_table(doc, ["Method", "ARI", "AMI", "Homogeneity", "Completeness", "V-measure"], [
        ["Geometry-only (non-learned, same idea as P2)", "43.49", "55.40", "52.48", "99.03", "60.96"],
    ...
    doc.add_paragraph(
        ...
        "THẬT đã có từ P1/P2 trong dự án:"
    )
    add_table(doc, ["Component", "Cost"], [
        ...
        ["CNN Re-ID feature extraction (REQUIRED, every detection, every frame, BOTH cameras)",
         "Same CATEGORY of cost already measured REAL in P1: adding OSNet x0.5 (chosen as the "
        ...
        ["Graph construction + Message Passing (PyTorch Geometric, pytorch-scatter/sparse)",
         "ADDITIONAL overhead on top of the CNN — not present in P2's pipeline (P2 only needs "
         "numpy/scipy)."],
    ...
    doc.add_paragraph(
        "Đối chiếu: bước ghép cặp CỦA P2 (thuần numpy/scipy Hungarian, KHÔNG cần CNN nào) đo THẬT "
        "trên Jetson Nano 4GB: 9.6ms cho 100 lần lặp ghép cặp 10×10 người/camera (kịch bản xấu "
        "nhất giả lập, vượt xa nhu cầu thật của P2 chỉ 1-2 người) — với đúng quy mô thật (1-2 "
...
def build_doc_compare():
    doc = Document()
    doc.add_heading("P2 — Cross-Camera Identity Association: So sánh Bài của nhóm vs Đối thủ", level=0)

    add_heading(doc, "1. Bảng so sánh trực tiếp")
    doc.add_paragraph(
        "So sánh 3 nguồn: (a) Geometry-only và GNN-CCA đầy đủ — số liệu LITERATURE, trích dẫn "
        "trực tiếp từ Table V của paper, đo trên EPFL (9 người, 4 camera); (b) P2 Hungarian — đo "
        "THẬT trên data P2 Data 3 của nhóm (1-2 người, 2 camera), ĐẦY ĐỦ gốc + 4 biến thể augment "
        "an toàn (sáng/tối/nhiễu/nén, giống quy ước P1/P3), tổng 840 frame. Cùng bộ metric (ARI/"
        "AMI/V-measure), KHÔNG cùng bộ data — ghi rõ để không gây hiểu nhầm là 'chạy trực tiếp "
        "cùng test set'."
    )
    add_table(doc, ["Metric", "Geometry-only (paper, EPFL)", "GNN-CCA full (paper, EPFL)", "P2 Hungarian (Ours, n=840)"], [
    ...
    doc.add_paragraph(
        "P2 (thuần hình học, KHÔNG train) đo trên data thật của nhóm (n=840, gốc + 4 augment) đạt "
    ...
    doc.add_paragraph(
        ...
        "thật khiến P1 phải chọn cẩn thận model Re-ID nhẹ nhất) CỘNG THÊM PyTorch Geometric "
        "(dependency khó cài trên Jetson Nano 4GB thật) — trong khi P2 đo thật chỉ tốn 9.6ms cho "
    ...
    doc.add_paragraph(
        "(a) KHÔNG train/chạy được bản GNN-CCA học đầy đủ trên chính data P2 — so sánh dừng ở "
        "mức trích dẫn số liệu literature (EPFL) đối chiếu với số đo thật trên data mình, KHÔNG "
        "phải chạy 2 phương pháp trên CÙNG 1 test set. Nếu chạy được GNN-CCA thật trên data P2, "
        "kết quả có thể khác (có thể còn cao hơn, vì quy mô nhỏ dễ hơn cho GNN).\n"
        "(b) Ground truth cho đánh giá trên data P2 là bootstrap từ tần suất ghép Hungarian (xem "
        ...
        "trúc thật + số đo Jetson thật của P1/P2), KHÔNG phải benchmark trực tiếp."
    )

    add_file_catalog(doc, [
        ("chart_clustering_vs_literature.png", "Biểu đồ so sánh ARI/AMI/V-measure: Geometry-only "
         "(paper) vs GNN-CCA đầy đủ (paper) vs P2 Hungarian (data thật của nhóm)."),
        ("Noi_dung.docx", "Chính file này."),
    ])
