...
def main():
    ...
    state = torch.load(RESULTS_DIR / "reid_head_pure_triplet.pt", map_location=DEVICE)
    ...
    df.to_csv(RESULTS_DIR / "reid_pure_triplet_tang2_results.csv", index=False, encoding="utf-8-sig")
    tpr = (df["predicted_own_model"] == df["expected"]).sum() / len(df) if len(df) else 0.0

    print("\n" + "=" * 60)
    print(f"Re-ID Head TU TRAIN (Pure Triplet) -- True Positive Rate: {tpr:.2%}")
    print("=" * 60)
    print(f"\nDa luu: {RESULTS_DIR / 'reid_pure_triplet_tang2_results.csv'}")
